class Todo {
  selectors = {
    ...
    searchTaskInput: '[data-js-todo-search-task-form]',
  ...
  /* Хранит CSS-классы состояний */
  stateClasses = {
    isVisible: 'is-visible', /* All button */
    isDisappearing: 'is-disappearing', /* Для удаляемого элемента */
  }

  localStorageKey = 'todo-items' /* Получаем и сохраняем данные из браузерного локального хранилища*/

  constructor() {
    this.rootElement = document.querySelector(this.selectors.root) /* Ищем главный модуль To-Do по селектору 'data-js-todo' */
    ...
    this.state = { /* Хранилище состояний To Do */
      items: this.getItemsFromLocalStorage(), /* Массив с данными для каждого To Do элемента */
      filteredItems: null, /* это свойство отвечает за отфильтрованные элементы после совершения поиска */
  ...
  render() {
    this.totalTasksElement.textContent = this.state.items.length /* обновляем число задач в текущем списке */

    this.deleteAllButtonElement.classList.toggle(
      this.stateClasses.isVisible,
      this.state.items.length > 0
    ) /* отображаем или скрываем кнопку "Delete All" */
    ...
    this.emptyMessageElement.textContent =
      isEmptyFilteredItems ? 'Tasks not found' /* задачи не найдены */
        : isEmptyItems ? 'There are not tasks yet' /* пока задач нет */
        : ''
  }

/* Методы для манипуляции полем Items в state-объекте */
  ...
  addItem(title) {
    ...
    this.saveItemsToLocalStorage() /* обновляет дянные в локальном браузерном хранилище */
    this.render() /* обновляет DOM-дерево в соответствие с новыми данными в state-объекте */
  }
